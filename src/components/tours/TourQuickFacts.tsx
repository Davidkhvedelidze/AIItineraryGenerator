...
export function TourQuickFacts({ tour }: TourQuickFactsProps) {
  ...
  const facts: QuickFact[] = [
    ...
    {
      label: "Starts",
      value: tour.startingLocation,
      icon: MapPin,
      tone: "bg-sky-50",
    },
    {
      label: "Ends",
      value: tour.endingLocation,
      icon: Route,
      tone: "bg-amber-50",
    },
  ...
  return (
    <section aria-label="Tour quick facts" className="flex flex-wrap gap-3 ">
      {facts.map((fact) => {
        const Icon = fact.icon;
        return (
          <div
            key={fact.label}
            className="rounded-2xl border items-center justify-center bg-card p-4 flex shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          >
            <div
              className={`inline-flex rounded-xl p-2 ${fact.tone || "bg-secondary"}`}
            >
              <Icon className="h-5 w-5 text-amber-700" aria-hidden="true" />
            </div>
            <p className=" text-xs mr-1 font-semibold uppercase tracking-wide text-muted-foreground">
              {fact.label} :
            </p>
